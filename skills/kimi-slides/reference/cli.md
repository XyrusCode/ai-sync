...
```bash
kimi-slides package path/deck -o path/deck.pptx
kimi-slides package path/deck
```

The input to `package` must be a directory containing a `.pptd` file. This command packages and renders the `.pptd` directory into a `.pptx` file.

`-o, --output` is optional. If omitted, a `.pptx` file with the same name is generated next to the input:

```text
path/deck/ -> path/deck.pptx
```

The input directory structure is typically:
...
During packaging, the page list in the main `.pptd` file is read and the corresponding `.page` files are loaded in order. Local resources referenced by pages, such as images and fonts, must be accessible relative to the input directory:

```yaml
src: ./images/<hash>.png
```

If there are problems such as YAML syntax errors, missing required fields, nonexistent resource files, or out-of-bounds elements, `package` may fail, or the generated `.pptx` may not match expectations.
